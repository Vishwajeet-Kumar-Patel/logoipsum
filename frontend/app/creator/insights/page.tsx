...
export default function InsightsPage() {
   ...
   useEffect(() => {
      const fetchInsights = async () => {
         try {
            ...
            setData({ ...dashRes.data, users: subsRes.data });
         } catch (err) {
   ...
   const audienceData = (data?.users || []).map((user: any) => ({
      id: user._id,
      name: user.username,
      email: `${user.username.toLowerCase().replace(' ', '')}@fan.com`,
      tier: user.subscriptionTier || 'Standard',
      joinDate: new Date(user.joinDate).toLocaleDateString(),
      endDate: 'N/A',
      img: `https://i.pravatar.cc/150?u=${user.userId}`
   }));

   const topSelling = (data?.postRevenueBreakdown || []).map((post: any) => ({
      title: post.title,
   ...
   const salesData = (data?.revenueChart || []).map((sale: any, i: number) => ({
      id: `sale-${i}`,
      name: `User ${i + 1}`,
      product: sale.title || 'Exclusive Post',
   ...
   const postPerformance = (data?.postRevenueBreakdown || []).map((post: any) => ({
      id: post.id,
      title: post.title,
      renown: Math.floor(post.total * 1.5),
      other: Math.floor(post.total * 0.5),
      img: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=100&q=80'
   }));

   const liveInsights: any[] = []; // Will be populated when livestreams are implemented
   ...
   return (
      <div className="p-12 max-w-7xl w-full mx-auto bg-[#f9f9f9] min-h-screen font-[var(--font-figtree)]" onClick={() => setActiveMenu(null)}>
         ...
         <header className="mb-12 flex justify-between items-start">
            <div>
               <h1 className="text-[52px] font-black text-[#1c1917] tracking-tighter mb-2 font-['Fjalla One']">Insights</h1>
         ...
         <div className="space-y-10">
                              ...
                              <tr key={user.id} className="hover:bg-slate-50 transition-colors cursor-pointer group">
                                 <td className="px-8 py-5">
                                    <div className="flex items-center gap-3">
                                       ...
                                       <span className="text-[14px] font-bold text-[#1c1917] group-hover:text-rose-500 transition-colors uppercase font-['Fjalla One']">{user.name}</span>
                                    </div>
                                 </td>
            ...
            {activeTab === 'Sales' && (
               <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl font-['Fjalla One']">
                              ...
                              <tr key={sale.id} className="hover:bg-slate-50 transition-all cursor-pointer group">
                                 <td className="px-8 py-5">
                                    <div className="flex items-center gap-3">
                                       ...
                                       <span className="text-[14px] font-bold text-[#1c1917] group-hover:text-rose-500 transition-colors uppercase font-['Fjalla One']">{sale.name}</span>
                                    </div>
                                 </td>
                                 ...
                                 <td className="px-8 py-5 text-[14px] font-black text-[#1c1917] font-['Fjalla One']">{sale.price}</td>
                  ...
                  <div className="space-y-6 pt-6">
                     <h2 className="text-[32px] font-black text-[#111827] tracking-tight font-['Fjalla One']">Top selling</h2>
                        ...
                        {topSelling.map((item: any, i: number) => (
                           <div key={i} className="bg-white border border-slate-200/60 rounded-3xl p-6 flex justify-between items-center hover:shadow-sm transition-all group cursor-pointer">
                              <div className="flex items-center gap-5">
                                 ...
                                 <div>
                                    <p className="text-[17px] font-black text-[#111827] group-hover:text-rose-500 transition-colors font-['Fjalla One']">{item.title}</p>
                              ...
                              <div className="text-right">
                                 <p className="text-[20px] font-black text-emerald-600 leading-tight font-['Fjalla One']">{item.revenue}</p>
            ...
            {activeTab === 'Memberships' && (
               <>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6 font-['Fjalla One']">
                     {[
                  ...
                  <div className="bg-white border border-slate-200 rounded-[48px] p-10 shadow-sm overflow-hidden">
                     <div className="flex justify-between items-end mb-12">
                        <div>
                           <h2 className="text-3xl font-black text-[#111827] mb-1 font-['Fjalla One']">Revenue Through Offers</h2>
                     ...
                     <div className="h-80 w-full relative group">
                        <div className="absolute inset-0 border-b border-slate-200/60 flex items-end justify-between px-4 pb-2">
                           {[20, 21, 22, 23, 24, 25, 26, 27].map((d) => (
                              <span key={d} className={`text-[13px] font-black font-['Fjalla One'] ${d === 24 ? 'text-emerald-600' : 'text-slate-400'}`}>{d}</span>
                           ))}
                        </div>
                        ...
                        <div className="absolute left-[60%] top-1/2 -translate-y-1/2 bg-[#d94828] text-white p-6 rounded-[32px] shadow-2xl z-20 min-w-[220px] border border-white/20">
                           <p className="text-[13px] font-bold opacity-80 mb-2 font-['Fjalla One']">20 March, 2026</p>
                           <p className="text-lg font-black mb-1 leading-tight font-['Fjalla One']">Membership name</p>
                           ...
                           <p className="text-xl font-black font-['Fjalla One']">Revenue : $600</p>
                        </div>
                     </div>
                  </div>
               </>
            ...
            {activeTab === 'Posts' && (
               <div className="space-y-12">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-['Fjalla One']">
                     <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
                  ...
                  <div className="space-y-6">
                     <h2 className="text-[32px] font-black text-[#111827] tracking-tight font-['Fjalla One']">Per post performnace</h2>
                     <div className="bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-sm">
                        <table className="w-full text-left">
                           <thead className="bg-[#fafafa] border-b border-slate-200">
                              <tr>
                           ...
                           <tbody className="divide-y divide-slate-100/80">
                              {postPerformance.map((post: { id: React.Key | null | undefined; img: string | Blob | undefined; title: string | number | bigint | boolean | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | Promise<string | number | bigint | boolean | React.ReactPortal | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | null | undefined> | null | undefined; renown: string | number | bigint | boolean | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | Promise<string | number | bigint | boolean | React.ReactPortal | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | null | undefined> | null | undefined; other: string | number | bigint | boolean | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | Promise<string | number | bigint | boolean | React.ReactPortal | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | null | undefined> | null | undefined; }) => (
                                 <tr key={post.id} className="hover:bg-slate-50 transition-colors">
                                    <td className="px-8 py-4">
                                       <div className="flex items-center gap-4">
                                          <img src={post.img} className="w-10 h-10 rounded-xl object-cover" alt="Post" />
                                          <span className="text-sm font-bold text-[#1c1917] font-['Fjalla One'] uppercase">{post.title}</span>
                                       </div>
                                    </td>
                  ...
                  <div className="space-y-6 pt-6 text-slate-800">
                     <h2 className="text-[32px] font-black text-[#111827] tracking-tight font-['Fjalla One']">Live event insights</h2>
                     <div className="bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-sm">
                        <table className="w-full text-left">
                           <thead className="bg-[#fafafa] border-b border-slate-200">
                              <tr>
                                 ...
                                 <tr key={live.id} className="hover:bg-slate-50 transition-colors">
                                    <td className="px-8 py-4">
                                       <div className="flex items-center gap-4">
                                          ...
                                          <span className="text-sm font-bold text-[#1c1917] font-['Fjalla One'] uppercase">{live.title}</span>
                                       </div>
                                    </td>
